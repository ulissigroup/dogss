...
class CrystalGraphConvNet(nn.Module):
    ...
    def __init__(self, orig_atom_fea_len, nbr_fea_len,
                 atom_fea_len=64, n_conv=3, h_fea_len=128, h_fea_len_dist=128, h_fea_len_const=128, h_fea_len_D=128, n_h_dist=1, n_h_const=1, n_h_D = 1,max_num_nbr=12, momentum=0.9, classification=False, max_opt_steps=300, min_opt_steps=10, opt_step_size=0.3, radius = 6, step_size=0.4, energy_mode="Harmonic",E_hea_len=83,grad_hea_len=83,n_h_grad=1):
        """
        ...
        self.bond_distance2 = nn.Linear(h_fea_len_dist, E_hea_len)
        self.bond_constant2 = nn.Linear(h_fea_len_const, E_hea_len)
        ...
        if self.energy_mode == "Morse" or self.energy_mode == "LJ":
            ...
            if n_h_D > 1:
                ...
                self.D_bn = nn.ModuleList([nn.BatchNorm1d(h_fea_len_D)
                                                 for _ in range(n_h_D-1)])
            self.D_constant = nn.Linear(h_fea_len_D, E_hea_len)
        ...
        self.grad_layer = nn.Linear(E_hea_len, grad_hea_len)
        self.grad_bn2 = nn.BatchNorm1d(grad_hea_len)
        self.grad_softplus = nn.Softplus()
        ...
        self.grad_pos = nn.Linear(grad_hea_len, 3)
        self.grad_pos_bn = nn.BatchNorm1d(3)
        self.grad_tanh = nn.Tanh()
...
#                 bond_energy = torch.abs(bond_constant*(bond_distance-distance)**2.) 
        ...
        grad = torch.mean(potential_E, dim=1) # N,M,C --> N,C
        grad = self.grad_softplus(self.grad_bn2(self.grad_layer(grad)))

        if hasattr(self, 'grad_fcs') and hasattr(self, 'grad_softpluses'):
            for fc, softplus,bn in zip(self.grad_fcs, self.grad_softpluses, self.grad_bn):        
                grad = softplus(bn(fc(grad)))

        grad = self.grad_pos(grad) #N,3
        grad = self.grad_tanh(self.grad_pos_bn(grad))
